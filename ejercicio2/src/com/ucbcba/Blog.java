...
public class Blog extends SitioWeb {
    ...
    public void anadirarticulos(Articulo articulo) throws ComentarioVacioException, ComentarioLengthException {
        if(articulo.getTexto().equals("")){
            throw  new ComentarioVacioException(articulo.getTexto());
        }
        if(articulo.getTexto().length() <= 50 && articulo.getTexto().length() >= 150 ){
            throw  new ComentarioLengthException(articulo.getTexto().length());
        }
    }
